...
namespace Web.Areas.Api.Controllers
{
    public class OutCourseController : Controller
    {
        ...
        /// 学习活动
        /// </summary>
        /// <param name="userId">用户编号（DES加密，URL编码）</param>
        /// <param name="classId">班级编号（DES加密，URL编码）</param>
        /// <param name="scoType">1：阅读 2：视频 3：测试 4：讨论 5：作业 6：考试 7：其他</param>
        /// <param name="recordData">评分信息（DES加密，URL编码）</param>
        /// <param name="timestamp">时间（精确到小时）+ 用户编号 + 班级编号，MD5加密</param>
        /// <returns></returns>
        [HttpPost]
        public ActionResult addRecordInfo(string userId, string classId, int scoType, string recordData, string timestamp)
        {
            ...
            #region 验证参数
            //userId = ApiTools.Decrypt(HttpUtility.UrlDecode(userId), aesKey, aesIV);
            ...
            try
            {
                outcourse = jss.Deserialize<OutCourse>(recordData);
            }
            catch (Exception)
            {
                result = false;
