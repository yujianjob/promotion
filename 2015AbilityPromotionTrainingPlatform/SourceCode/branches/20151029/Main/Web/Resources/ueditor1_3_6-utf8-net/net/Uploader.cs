...
namespace Dianda.AP
{
    ...
    public class Uploader
    {
      ...
      */
        public Hashtable upFile(HttpContext cxt, string pathbase, string[] filetype, int size)
        {
            ...
            try
            {
                ...
                if (state == "SUCCESS")
                {
                    ...
                    uploadFile.SaveAs(uploadpath + "locality" + testname);//标准用户上传图
                    //URL = pathbase + testname;
                    URL = pathbase + "slocality" + testname;
                    //URL = pathbase + testname;
                    System.Drawing.Image smallImage = GetNewImage(uploadpath + "locality" + testname, 120, 100);//订制的小图
                    smallImage.Save(uploadpath + "slocality" + testname);
                    //smallImage.Save(uploadpath + testname);
                }
            }
            catch (Exception)
            {
        ...
        /**
     * 上传附件的主处理方法(为了不影响图片缩略图效果,重新写一个方法)
     * @param HttpContext
     * @param string
     * @param  string[]
     *@param int
     * @return Hashtable
     */
        public Hashtable upFile1(HttpContext cxt, string pathbase, string[] filetype, int size)
        {
            pathbase = pathbase + "/";
            uploadpath = cxt.Server.MapPath(pathbase);//获取文件上传路径
            ...
            try
            {
                ...
                if (state == "SUCCESS")
                {
                    ...
                    uploadFile.SaveAs(uploadpath + testname);//标准用户上传图
                    URL = pathbase + testname;
                }
            }
            catch (Exception)
        ...
        {
            //得到小图片Image对象 
            System.Drawing.Image smallImage = GetNewImage(sourcePath, newWidth, newHeight);
        ...
        private bool IsTrue() { return true; }
