...
namespace Dianda.AP.DAL
{
	public class Course_OutCourseRecordDAL
	{
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append("select [Id],[ClassId],[TrainingId],[AccountId],[DataType],[learningTime],[Role],[IsPassed],[Remark],[Score],[Status],[Delflag],[CreateDate],[Title],[AssessmentName] from [dbo].[Course_OutCourseRecord]");
            if (!string.IsNullOrEmpty(where))
                sql.Append(" where " + where);
            if (!string.IsNullOrEmpty(orderBy))
                sql.Append(" order by " + orderBy);
        ...
        public DataTable GetTable(int pageSize, int pageIndex, string where, string orderBy)
        {
            ...
            int end = pageIndex * pageSize;
            StringBuilder sql = new StringBuilder();
            sql.Append("select * from (select [Id],[ClassId],[TrainingId],[AccountId],[DataType],[learningTime],[Role],[IsPassed],[Remark],[Score],[Status],[Delflag],[CreateDate],[Title],[AssessmentName],ROW_NUMBER() over (order by " + orderBy + ") as [RowNum] from [dbo].[Course_OutCourseRecord]");
            if (!string.IsNullOrEmpty(where))
                sql.Append(" where " + where);
            sql.Append(") as T where [RowNum] between " + start + " and " + end);
            return MSEntLibSqlHelper.ExecuteDataSetBySql(sql.ToString()).Tables[0];
