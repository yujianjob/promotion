...
namespace Dianda.AP.DAL
{
    public class AccountEditDAL
    {
        ...
        public DataTable GetAccountOrgan(int AccountId)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(" select OrganId,(select ParentId from Organ_Detail where Id = a.OrganId) as Region from Member_Account a where a.Id = '" + AccountId + "' ");
        ...
        public DataTable GetList(int AccountId)
        {
            ...
            sql.Append(" left join Organ_Detail e on e.id = (select ParentId from Organ_Detail where Id = a.OrganId) ");
            sql.Append(" left join Organ_Detail f on a.OrganId = f.Id ");
        ...
        public DataTable GetOutRegin(int Region,int Organ)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(" select * from Organ_Detail where OType = " + Region + " or OType = " + Organ + " and Delflag = 0 ");
            return MSEntLibSqlHelper.ExecuteDataSetBySql(sql.ToString()).Tables[0];
        }
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(" select Id,Title from Organ_Detail where ParentId = " + id + " and Delflag = 0 and Otype <> 5 ");
            return MSEntLibSqlHelper.ExecuteDataSetBySql(sql.ToString()).Tables[0];
        }
