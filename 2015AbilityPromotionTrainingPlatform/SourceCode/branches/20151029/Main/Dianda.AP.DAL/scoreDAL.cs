...
namespace Dianda.AP.DAL
{
	public class scoreDAL
	{
		...
		public int Add(score model)
		{
			...
			sql.Append("insert into [dbo].[score] ([userid],[dimensionid],[score],[num])");
		...
		public int Update(score model)
		{
			...
			sql.Append("[userid]=@userid,[dimensionid]=@dimensionid,[score]=@userscore,[num]=@num");
		...
		{
			StringBuilder sql = new StringBuilder();
			sql.Append("select [id],[userid],[dimensionid],[score],[num] from [dbo].[score]");
			if (!string.IsNullOrEmpty(where))
				sql.Append(" where " + where);
			if (!string.IsNullOrEmpty(orderBy))
				sql.Append(" order by " + orderBy);
		...
		public DataTable GetTable(int pageSize, int pageIndex, string where, string orderBy, out int recordCount)
		{
			...
			int end = pageIndex * pageSize;
			StringBuilder sql = new StringBuilder();
			sql.Append("select * from (select [id],[userid],[dimensionid],[score],[num],ROW_NUMBER() over (order by " + orderBy + ") as [RowNum] from [dbo].[score]");
			if (!string.IsNullOrEmpty(where))
				sql.Append(" where " + where);
			sql.Append(") as T where [RowNum] between " + start + " and " + end);
			return MSEntLibSqlHelper.ExecuteDataSetBySql(sql.ToString()).Tables[0];
		...
		private void ConvertToModel(IDataReader reader, score model)
		{
			...
			if (reader["score"] != DBNull.Value)
				model.userscore = Convert.ToDecimal(reader["score"]);
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append("select (case when (ISNUMERIC(score)=1) then cast(score as float)*100 else 0 end) as userscorefrom from [2015NLCP].dbo.score where userid =@id");
            SqlParameter[] cmdParams = new SqlParameter[]{
                new SqlParameter("@id", SqlDbType.Int, 4) { Value = id },
