...
namespace Dianda.AP.DAL
{
	public class Traning_OutCourseTypeDAL
	{
		...
		public int Add(Traning_OutCourseType model)
		{
            ...
            sql.Append("insert into [dbo].[Traning_OutCourseType] ([Title],[Link],[Delflag],[CreateDate],[DisplayEnterBtn])");
			...
			SqlParameter[] cmdParams = new SqlParameter[]{
                ...
                new SqlParameter("@DisplayEnterBtn", SqlDbType.Bit, 1) { Value = model.DisplayEnterBtn }
			};
		...
		public int Update(Traning_OutCourseType model)
		{
            ...
            sql.Append("[Title]=@Title,[Link]=@Link,[Delflag]=@Delflag,[CreateDate]=@CreateDate,[DisplayEnterBtn]=@DisplayEnterBtn");
		...
		{
			StringBuilder sql = new StringBuilder();
            sql.Append("select [Id],[Title],[Link],[Delflag],[CreateDate],[DisplayEnterBtn] from [dbo].[Traning_OutCourseType]");
			if (!string.IsNullOrEmpty(where))
				sql.Append(" where " + where);
			if (!string.IsNullOrEmpty(orderBy))
				sql.Append(" order by " + orderBy);
		...
		public DataTable GetTable(int pageSize, int pageIndex, string where, string orderBy)
		{
			...
			int end = pageIndex * pageSize;
			StringBuilder sql = new StringBuilder();
            sql.Append("select * from (select [Id],[Title],[Link],[Delflag],[CreateDate],[DisplayEnterBtn],ROW_NUMBER() over (order by " + orderBy + ") as [RowNum] from [dbo].[Traning_OutCourseType]");
			if (!string.IsNullOrEmpty(where))
				sql.Append(" where " + where);
			sql.Append(") as T where [RowNum] between " + start + " and " + end);
			return MSEntLibSqlHelper.ExecuteDataSetBySql(sql.ToString()).Tables[0];
