...
{
	public class Member_CourseContentTestAnswerDAL
	{
		/// <summary>
		/// 新增一条记录
		/// </summary>
		...
		{
			StringBuilder sql = new StringBuilder();
			sql.Append("select [Id],[AnswerResult],[Question],[Answer],[Credit],[Result],[AccountId],[Delflag],[CreateDate] from [dbo].[Member_CourseContentTestAnswer]");
			if (!string.IsNullOrEmpty(where))
				sql.Append(" where " + where);
			if (!string.IsNullOrEmpty(orderBy))
				sql.Append(" order by " + orderBy);
		...
		public DataTable GetTable(int pageSize, int pageIndex, string where, string orderBy, out int recordCount)
		{
			...
			int end = pageIndex * pageSize;
			StringBuilder sql = new StringBuilder();
			sql.Append("select * from (select [Id],[AnswerResult],[Question],[Answer],[Credit],[Result],[AccountId],[Delflag],[CreateDate],ROW_NUMBER() over (order by " + orderBy + ") as [RowNum] from [dbo].[Member_CourseContentTestAnswer]");
			if (!string.IsNullOrEmpty(where))
				sql.Append(" where " + where);
			sql.Append(") as T where [RowNum] between " + start + " and " + end);
			return MSEntLibSqlHelper.ExecuteDataSetBySql(sql.ToString()).Tables[0];
