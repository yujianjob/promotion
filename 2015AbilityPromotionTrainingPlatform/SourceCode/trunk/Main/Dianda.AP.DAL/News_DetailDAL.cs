...
namespace Dianda.AP.DAL
{
	public class News_DetailDAL
	{
        ...
        public bool YzTitle(string Title,int id)
        {
            ...
            sql.Append(" select count(1) from News_Detail where Title = @Title and Id <> @Id and Delflag = 0 ");
            SqlParameter[] para ={
                                  new SqlParameter("@Title",Title),
                                  new SqlParameter("@Id",id)
                                 };
            int count = Convert.ToInt32(MSEntLibSqlHelper.ExecuteScalarBySql(sql.ToString(), para));
            if (count > 0)
            {
                return true;
            }
		...
		public int Update(News_Detail model)
		{
			...
			sql.Append("[Title]=@Title,[Content]=@Content,[sort]=@sort,[OrganId]=@OrganId,[Creater]=@Creater,[PlanId]=@PlanId,[Display]=@Display,[PublishDate]=@PublishDate");
		...
		public List<News_Detail> GetList(int pageSize, int pageIndex, string where, string orderBy, out int recordCount)
		{
            ...
            sb.Append("select count(1) from [dbo].[News_Detail] a where Delflag = 0 ");
			if (!string.IsNullOrEmpty(where))
				sb.Append(where);
        ...
        public List<SelectListItem> GetPlan()
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.Append(" select id,Title from Training_Plan");
