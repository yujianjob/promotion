...
namespace Dianda.AP.BLL
{
    public class PrepareTrainingApplyBLL
    {
        ...
        public bool CheckExists(string title, int organId, int id)
        {
            ...
            return new PrepareTrainingApplyDAL().CheckExists(where.ToString());
        }
        ...
        /// <param name="where"></param>
        /// <returns></returns>
        public DataTable GetTeacherList(int pageSize, int pageIndex, string orderBy, int partitionId, string organ, string where)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("A.Delflag=0 and A.Status=2 and A.OrganId in (" + organ + ")");
            ...
            if (!string.IsNullOrEmpty(where))
                sb.Append(" and " + where);
            return new PrepareTrainingApplyDAL().GetTeacherList(pageSize, pageIndex, sb.ToString(), orderBy);
        }
        ...
        /// <param name="where"></param>
        /// <returns></returns>
        public int GetTeacherCount(int partitionId, string organ, string where)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("A.Delflag=0 and A.Status=2 and A.OrganId in (" + organ + ")");
            sb.Append(" and B.Delflag=0 and B.Status=1 and B.PartitionId=" + partitionId + " and B.GroupId=" + (int)PlatformGroupEnum.TeacherCourse);
