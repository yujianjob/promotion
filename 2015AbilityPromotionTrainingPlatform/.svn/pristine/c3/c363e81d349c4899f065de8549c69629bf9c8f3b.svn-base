$(function () {
    $proto.init(function () {
        ...
        $('#btnSubmit').click(function () {
            ...
            var obj = JSON.stringify({ UnitId: $("#UnitId").val(), Title: HtmlEncode($("#txtTitle").val()), Sort: $("#txtSort").val(), TimeLength: $("#txtTimeLength").val(), Content: preview.getContent(), OpenTime: ($("#radioPartialOpen").is(":checked") ? $("#txtAfterOpenDayNum").val() : 0), EndTime: ($("#radioPartialEnd").is(":checked") ? $("#txtAfterEndDayNum").val() : 0), UnitType: 3 });
            $.ajax({
                cache: false,
                type: 'post',
