$(function () {
    $proto.init(function () {
        ...
        $('#btnSubmit').click(function () {
            ...
            var obj = JSON.stringify({ TrainingId: TrainingId, ParentId: ($("#radioSection").is(":checked") ? $("#ParentId").val() : 0), Title: HtmlEncode($("#txtTitle").val()), Sort: $("#txtSort").val(), Content: HtmlEncode($("#areaContent").val()), Display: $("#chkDisplay").attr("checked") == "checked" ? true : false });
            $.ajax({
                cache: false,
                type: 'post',
