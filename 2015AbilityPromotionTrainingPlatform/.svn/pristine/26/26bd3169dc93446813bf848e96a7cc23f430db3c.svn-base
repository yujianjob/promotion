$(function () {
    $proto.init(function () {
        ...
        $('#btnSubmit').click(function () {
            ...
            var obj = JSON.stringify({ Id: Id, UnitId: UnitId, Title: HtmlEncode($("#txtTitle").val()), Sort: $("#txtSort").val(), TimeLength: $("#txtTimeLength").val(), Content: HtmlEncode($("#areaContent").val()), TestCnt: ($("#radioLimit").is(":checked") ? $("#txtTestCnt").val() : -1), PrintScore: ($("#radioPrintScore").is(":checked") ? 1 : 0), OpenTime: ($("#radioPartialOpen").is(":checked") ? $("#txtAfterOpenDayNum").val() : 0), EndTime: ($("#radioPartialEnd").is(":checked") ? $("#txtAfterEndDayNum").val() : 0), UnitType: 5 });
            $.ajax({
                cache: false,
                type: 'post',
