...
namespace Dianda.AP.DAL
{
    public class LearnMyCourseDAL
    {
        ...
        /// <returns></returns>
        public MyCourseInfo GetMyCourse(string where)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append("select ");
            sql.Append("A.Id,A.Status as StatusMember,A.Result,A.AccountId,A.CurrentSchedule,A.TotalSchedule,");
            sql.Append("B.Id as ClassId,B.Status as StatusClass,B.Title as ClassName,B.OpenClassFrom,B.OpenClassTo,");
        ...
        private void ConvertToModel(IDataReader reader, MyCourseInfo model)
        {
            ...
            if (reader["StatusMember"] != DBNull.Value)
                model.StatusMember = Convert.ToInt32(reader["StatusMember"]);
            if (reader["StatusClass"] != DBNull.Value)
                model.StatusClass = Convert.ToInt32(reader["StatusClass"]);
