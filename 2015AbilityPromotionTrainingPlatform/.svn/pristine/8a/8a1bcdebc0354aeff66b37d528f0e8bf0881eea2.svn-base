...
namespace Dianda.AP.DAL
{
    public class Sys_MessageDAL
    {
        ...
        {
            string sql = "select b.NickName,a.* from [dbo].[Sys_Message] a left join Member_Account b on a.AccountId = b.Id where a.Id=@Id";
        ...
        public List<Sys_Message> GetList(int pageSize, int pageIndex, string where, string orderBy, out int recordCount)
        {
            ...
            sb.Append("select count(1) from [dbo].[Sys_Message] a left join Member_Account b on a.AccountId = b.Id ");
            if (!string.IsNullOrEmpty(where))
            ...
            StringBuilder sql = new StringBuilder();
            sql.Append("select * from (select b.NickName,a.*,ROW_NUMBER() over (order by " + orderBy + ") as RowNum from [dbo].[Sys_Message] a left join Member_Account b on a.AccountId = b.Id");
            if (!string.IsNullOrEmpty(where))
                sql.Append(" where " + where);
