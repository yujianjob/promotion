...
namespace Dianda.AP.DAL
{
    public partial class Member_AccountDAL
    {
        ...
        {
            string sql = "select * from [dbo].[v_account] where [Id]=@Id";
            if (!string.IsNullOrEmpty(where))
                sql += " and " + where;
            SqlParameter[] cmdParams = new SqlParameter[]{
				new SqlParameter("@Id", SqlDbType.Int, 4) { Value = id }
            ...
            if (Type == 1)
            {
                if (Groupid == 2 || Groupid == 4)
                {
                    ...
                    if (ClassModel.OrganRange != "0")
                    {
                        ...
                        s.Append(" or ma.OrganId in(select Id from Organ_Detail where charindex(','+cast(ParentId as varchar)+',',','+(select OrganRange from Class_Detail where Id='" + ClassModel.Id + "')+',')>0) )");
