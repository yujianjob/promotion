$(function () {
    $proto.init(function () {
        ...
        $('#btnSubmit').click(function () {
            ...
            if (IsActivityEditRename(UnitId, 4, Trim(HtmlEncode($("#txtTitle").val())), Id)) {
            ...
            var obj = JSON.stringify({ Id: Id, UnitId: UnitId, Title: HtmlEncode($("#txtTitle").val()), Sort: $("#txtSort").val(), TimeLength: $("#txtTimeLength").val(), Content: HtmlEncode($("#areaContent").val()), OpenTime: ($("#radioPartialOpen").is(":checked") ? $("#txtAfterOpenDayNum").val() : 0), EndTime: ($("#radioPartialEnd").is(":checked") ? $("#txtAfterEndDayNum").val() : 0), UnitType: 4 });
            $.ajax({
                cache: false,
                type: 'post',
