...
namespace Dianda.AP.DAL
{
    public class Training_CreditsDAL
    {
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(" select Title,Id from dbo.Traning_Field order by Id ");
        ...
        public DataTable GetAvgCredits(int Id,int PlanId)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(" select id,MinValue from Training_Credits where TraningField = -1 and OrganId = " + Id + " and PlanId = '" + PlanId + "' ");
            return MSEntLibSqlHelper.ExecuteDataSetBySql(sql.ToString()).Tables[0];
        }
